...
export interface Note {
  ...
  address?: string;
  /** Город локации — по нему места группируются в справочнике. */
  city?: string;
...
/** Состояние задачи помимо галочки «сделано».
 *  `active` — в работе; `someday` — замысел из бэклога, до которого дойдут
 *  когда-нибудь (в списке дня не мешается); `waiting` — мяч на чужой стороне,
 *  сделать нечего, но забывать нельзя. */
export type TaskStatus = 'active' | 'someday' | 'waiting';

/** Важность задачи. Отдельно от срока: срочное ≠ важное. */
 ...
 *
 *  Список задаёт ОДНО измерение (сферу), поэтому остальные живут прямо на
 *  пункте: теги (тип действия), статус, важность и размер. Все поля
 *  необязательные — задачи, заведённые до их появления, остаются валидными и
 *  читаются как «active / normal / без тегов». */
export interface ChecklistItem {
  ...
  /** Теги — второе измерение поверх списка: тип действия, контекст, что угодно.
   *  Хранятся без решётки, в нижнем регистре. */
  tags?: string[];
  /** Состояние (по умолчанию 'active'). */
  status?: TaskStatus;
  /** Кого или чего ждём — заполняется при статусе 'waiting'. */
  waitingFor?: string;
  /** Важность (по умолчанию 'normal'). */
  priority?: TaskPriority;
  /** Во сколько минут задача обойдётся: 5 / 15 / 60 … Нужен для вопроса
   *  «что закрыть прямо сейчас за 15 минут». */
  sizeMin?: number;
  /** Повтор: при отметке «сделано» задача не закрывается, а уезжает на
   *  следующий срок (как повтор события, но с галочкой). */
  repeat?: Repeat;
